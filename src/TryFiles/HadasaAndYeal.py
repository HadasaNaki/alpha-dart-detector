...
def frangiFilter_accuracy(file_binary_mask):
    ...
    # חישוב טווח הסיגמות
    min_scale = max(1, math.ceil(target_radius_voxels)-2)
    max_scale = math.ceil(target_radius_voxels)+2
    sigmas = np.arange(min_scale, max_scale+1)  # יוצר מערך [min_scale, min_scale+1, ..., max_scalel]
    
    #frangi defines - עם הפרמטר החדש
    frangi_result = frangi(
        volume,
        sigmas=sigmas,          # השתמש ב-sigmas במקום scale_range ו-scale_step
...
def pixelSize_accuracy(file_binary_mask):
    # טען את הקובץ
    nii = nib.load(file_binary_mask)
    binary_mask = nii.get_fdata().astype(np.uint8)
    ...
    structure = np.ones((3,3,3), dtype=np.int32)  # מבנה קישוריות 26 סמוכים
...
#     dicom_dir= r"C:\Users\yosef\Downloads\AlphaTauDataSet\2023-11__Studies"
...
if __name__ == "__main__":
    ...
    output_dir = r"C:\Users\yosef\Downloads\AlphaTauDataSet\output"
    os.makedirs(output_dir, exist_ok=True)
    
    # שמות הקבצים
    raw_nii_file = os.path.join(output_dir, "raw_volume.nii.gz")
    normalized_nii_file = os.path.join(output_dir, "normalized_image.nii.gz")
    needle_mask_file = os.path.join(output_dir, "needle_mask.nii.gz")
    
    # שמות קבצים לפילטרים השונים
    frangi_output = os.path.join(output_dir, "frangi_filtered.nii.gz")
    pixel_size_output = os.path.join(output_dir, "pixel_size_filtered.nii.gz")
    solidity_output = os.path.join(output_dir, "solidity_filtered.nii.gz")
    aspect_ratio_output = os.path.join(output_dir, "aspect_ratio_filtered.nii.gz")
    axis_length_output = os.path.join(output_dir, "axis_length_filtered.nii.gz")
    
    print("\n=== שלב 1: המרת DICOM ל-NIfTI ===")
    raw_nii_file, window_width, window_level, slope, intercept = process_dicom_to_nifti(dicom_dir, raw_nii_file)
    
    print("\n=== שלב 2: נרמול התמונה ===")
    normalized_data = normalize_nii_with_window(
        nii_file_path=raw_nii_file,
        window_width=window_width,
        window_level=window_level,
        output_path=normalized_nii_file,
        show_slices=True,
        rescale_slope=slope,
        rescale_intercept=intercept
    )
    
    print("\n=== שלב 3: יצירת מסיכת מחטים בינארית ===")
    binary_mask, thresholds = create_needle_mask_with_multi_otsu(
        nii_file_path=normalized_nii_file,
        num_classes=3,
        select_brightest=True,
        output_path=needle_mask_file,
        show_results=True
    )
    
    print("\n=== שלב 4: הפעלת פילטרים מתקדמים ===")
    
    # פילטר Frangi לזיהוי מבנים צינוריים
    print("\nמפעיל פילטר Frangi...")
    frangi_buffer = frangiFilter_accuracy(needle_mask_file)
    save_buffer_as_nii(frangi_buffer, frangi_output)
    
    # פילטר גודל פיקסלים
    print("\nמפעיל פילטר גודל פיקסלים...")
    pixel_size_buffer = pixelSize_accuracy(needle_mask_file)
    save_buffer_as_nii(pixel_size_buffer, pixel_size_output)
    
    # פילטר מוצקות
    print("\nמפעיל פילטר מוצקות...")
    solidity_buffer = soldit_accuracy(needle_mask_file)
    save_buffer_as_nii(solidity_buffer, solidity_output)
    
    # פילטר יחס אורך-רוחב
    print("\nמפעיל פילטר יחס אורך-רוחב...")
    aspect_ratio_buffer = aspect_ratio_accuracy(needle_mask_file)
    save_buffer_as_nii(aspect_ratio_buffer, aspect_ratio_output)
    
    # פילטר אורך ציר
    print("\nמפעיל פילטר אורך ציר...")
    axis_length_buffer = axisLen_accuracy(needle_mask_file)
    save_buffer_as_nii(axis_length_buffer, axis_length_output)
